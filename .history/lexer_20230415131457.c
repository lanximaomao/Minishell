...
t_list *get_linelst(char *line)
{
	...
	while (line[++i])
	{
		if (!(line[i] == ' ' || (line[i] > 8 && line[i] < 14)))
		{
			...
			while (!(line[i] == ' ' || (line[i] > 8 && line[i] < 14)) && line[i] && ++len)
			{
				...
				if (line[i] == '2')
				{
					if (line[i + 1] == '<' && line[i + 2] != '<')
						input->redir_sign = 5; // errinfile
					else if (line[i + 1] == '<' && line[i + 2] == '<')
						input->redir_sign = 6; // errheredoc
					else if (line[i + 1] == '>' && line[i + 2] != '>')
						input->redir_sign = 7; // erroutfile
					else if (line[i + 1] == '>' && line[i + 2] == '>')
						input->redir_sign = 8; // errappend
					len--;
					break;
				}
			...
			if (input->redir_sign == 2 || input->redir_sign == 4 || input->redir_sign = 5 || input->redir_sign = 5)
