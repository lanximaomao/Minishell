...
t_list *get_linelst(char *line)
{
	...
	while (line[++i])
	{
		if (!(line[i] == ' ' || (line[i] > 8 && line[i] < 14)))
		{
			...
			{
				ft_error("\033[1;31mError!\033[0m: %s", "Malloc failed\n");
			...
			while (!(line[i] == ' ' || (line[i] > 8 && line[i] < 14)) && line[i] && ++len)
			{
				...
				if (line[i] == '<') // 函数内部可以直接判断后面有没有东西，没有的话syntax error，或者在parser上做
				{
					...
					{
						ft_printf("\033[1;31mError!\033[0m: %s", "syntax error: more then two '<'\n");
						return (NULL);
					}
					// len--;
					break;
				...
				if ((line[i] == '\'' && (input->quote_type = 1)) || (line[i] == '\"' && (input->quote_type = 2)))
				{
					...
					{
						ft_printf("\033[1;31mError! \033[0m %s", "syntax error: quote not closed\n");
						return (NULL);
					}
				}
				i++;
