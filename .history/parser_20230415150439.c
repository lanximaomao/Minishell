...
t_list *parse_tokens(t_list *line_lst)
{
	...
	while (line_lst)
	{
		while (((t_input *)line_lst->content)->pipe_sign != 1)
		{
			...
			if (((t_input *)line_lst->content)->redir_sign == 5 || ((t_input *)line_lst->content)->redir_sign == 6)
			{
				// error
				if (((t_input *)line_lst->content)->redir_sign == 1 || ((t_input *)line_lst->content)->redir_sign == 8)
				{
