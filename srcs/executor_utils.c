...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/10 17:21:14 by lliu              #+#    #+#             */
/*   Updated: 2023/07/31 16:04:09 by lsun             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	create_child(int *pid, int i)
{
	pid[i] = fork();
	if (pid[i] == -1)
		ft_error(" minishell: fork failed", 4, 0);
	return (pid[i]);
}
...
char	*get_path_cmd(char *str, t_list *env)
{
	...
	while (path_env[i])
	{
		...
		if (!path_cmd)
			ft_error(" minishell: string join failed. \n", 1, 0);
...
char	**get_path_env(t_list *env)
{
	...
	if (!path_env)
		ft_error(" minishell: malloc fail or PATH is NULL.\n", 1, 0);
	return (path_env);
}
