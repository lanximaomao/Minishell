...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2023/06/10 17:21:21 by lliu              #+#    #+#             */
/*   Updated: 2023/07/31 16:04:13 by lsun             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int	executor(t_mini *mini, int size)
{
	...
	if (!pid)
		ft_error(" minishell: pid malloc fail", 1, 0);
	status = malloc(sizeof(int) * size);
	if (!status)
		ft_error(" minishell: status malloc fail", 1, 0);
